  ...
  return (
    <div className="flex flex-col h-full bg-transparent overflow-y-auto  [&::-webkit-scrollbar]:hidden">
      ...
      <div className="bg-[#111827] px-4 pt-4 pb-3 flex justify-between items-center shadow-sm z-10 sticky top-0 border-b border-white/5">
        ...
        <Link href="/dashboard/mining" className="w-8 h-8 bg-gradient-to-r from-[#4f8cff] to-[#6ee7ff] rounded-md flex items-center justify-center text-white hover:opacity-95 transition-all shadow-sm cursor-pointer">
          <Plus size={16} />
        </Link>
      </div>
      ...
      <div className="px-4 pt-4 space-y-4 max-w-[480px] mx-auto w-full pb-10">
        ...
        <div className="flex p-1 bg-[#111827] rounded-[12px] border border-white/5 shadow-sm">
          <button
            ...
            className={`flex-1 py-2 rounded-[8px] text-[13px] font-bold transition-colors cursor-pointer ${activeTab === "active"
                ? "bg-gradient-to-r from-[#4f8cff] to-[#6ee7ff] text-white shadow-sm"
        ...
        {/* Stats Card */}
        <div className="bg-gradient-to-r from-[#4f8cff] to-[#6ee7ff] rounded-2xl px-5 py-4 text-white shadow-[0_4px_14px_rgba(79,140,255,0.3)] flex justify-between items-center">
          <div className="text-center flex-1">
            <div className="text-[18px] font-bold tracking-tight">{settings.currency_symbol || "R"}{Number(investments.stats.total_invested).toFixed(2)}</div>
            <div className="text-[11px] text-white/80 font-medium">Total Invested</div>
          </div>
          <div className="text-center flex-1 border-x border-white/20">
            ...
            <div className="text-[11px] text-white/80 font-medium capitalize">Active</div>
          </div>
          <div className="text-center flex-1">
            <div className="text-[18px] font-bold tracking-tight">{settings.currency_symbol || "R"}{Number(investments.stats.est_monthly).toFixed(2)}</div>
            <div className="text-[11px] text-white/80 font-medium">Est. Monthly</div>
          </div>
        </div>
            ...
            {currentList.map((inv) => {
              ...
              return (
                <div key={inv.id} className="bg-[#111827] rounded-[16px] p-4 border border-white/5 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-[#4f8cff]"></div>
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className={`px-2 py-1 rounded text-[10px] font-bold ${activeTab === 'active' ? 'bg-blue-500/10 text-[#4f8cff]' : 'bg-green-500/10 text-green-400'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-white/5 p-2 rounded-lg">
                      ...
                      <div className="text-white font-bold text-[13px]">{settings.currency_symbol || "R"}{Number(inv.amount).toFixed(2)}</div>
                    </div>
                    <div className="bg-white/5 p-2 rounded-lg">
                      <div className="text-gray-400 text-[10px] mb-1">Expected Return</div>
                      <div className="text-green-400 font-bold text-[13px]">
                        {inv.daily_profit ? `${settings.currency_symbol || "R"}${Number(inv.daily_profit).toFixed(2)}/Day` : inv.plan?.daily_income ? `${settings.currency_symbol || "R"}${Number(inv.plan.daily_income).toFixed(2)}/Day` : 'N/A'}
                      </div>
                    </div>
                  </div>
